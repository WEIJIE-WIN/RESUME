...
if __name__ == '__main__':
    ...
    if "duie" in args.model_file:
        data = "DUIE"
    elif "nyt" in args.model_file:
        data = "NYT"
    elif "resume" in args.model_file:
        data = "RESUME"
    ...
    elif "roberta" in args.model_file:
        input_size = 768
        args.embed_mode = "roberta"
    ...
    with open("data/" + data + "/ner2idx.json", "r",encoding='utf-8') as f:
        ner2idx = json.load(f)
    with open("data/" + data+ "/rel2idx.json", "r",encoding='utf-8') as f:
        rel2idx = json.load(f)
    ...
    ner_score, re_score,_ = model(target_sent, mask)
    ...
    for en in entity:
        ...
        entity_str = " ".join(target_sent[start_offset:end_offset+1])
        entity_names[entity_str] = start_offset
        print("entity_name: {}, entity type: {}".format(entity_str, lable))
